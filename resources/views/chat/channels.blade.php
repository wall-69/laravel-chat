...
@section('content')
    ...
    <main>
        <div class="d-flex flex-column flex-md-row px-4 py-3 gap-1 justify-content-between">
            {{-- Search --}}
            <form method="GET" action="{{ route('chats.channels') }}" class="d-flex flex-column">
            ...
            {{-- Create new channel --}}
            <a href="{{ route('chats.create') }}" class="bg-primary px-2 py-2 text-center my-auto text-white rounded-3">
                Create new channel
            </a>
        </div>
                        ...
                        <article class="d-flex justify-content-between bg-primary p-2 rounded-3"
                            ...
                            <div class="d-flex flex-column align-items-start justify-content-between">
                                ...
                                @if (auth()->user()->userChats->doesntContain('chat_id', $channel->id))
                                    <form method="POST" action="{{ route('chats.join', $channel->id) }}">
                                        @method('POST')
                                        @csrf
